...
class ELModel(nn.Module):
    ...
    def __init__(self, nb_classes, nb_relations, embedding_size, batch_size, margin, reg_norm):
        ...
        rel_constants = np.random.uniform(low=-1, high=1, size=(nb_relations, embedding_size + 1))
    ...
    def nf1_loss(self, input):
        ...
        r = input[:,1]
        d = input[:, 2]
        ec = self.cls_embeddings(c)
        ed = self.cls_embeddings(d)

        rc = torch.max(torch.tensor([0.0]).cuda(), ec[:, -1])
        rd = torch.max(torch.tensor([0.0]).cuda(), ed[:, -1])
...
#         x3 = x1 + r
        ...
        return dst + self.reg(ec[:, 0:-1]) + self.reg(ed[:, 0:-1])
    ...
    def nf2_loss(self, input):
        ...
        re = torch.max(torch.tensor([0.0]).cuda(), e[:, -1])
        sr = rc + rd
...
#         rc = F.relu(c[:, -1])
#         rd = F.relu(d[:, -1])
        rc = torch.max(torch.tensor([0.0]).cuda(), cr[:, -1])
        rd = torch.max(torch.tensor([0.0]).cuda(), dr[:, -1])
...
#         loss = loss1 + loss2 + loss3 + loss4 + loss_top + loss_nf3_neg + loss5 + loss6 - loss7
        ...
        #         print(loss1.shape, loss2.shape, loss5.shape)
#         print(loss.shape, '\n', torch.mean(loss1.squeeze()), torch.mean(loss2.squeeze()), torch.mean(loss3.squeeze()), torch.mean(loss4.squeeze()),  torch.mean(loss_top.squeeze()), torch.mean(loss_nf3_neg.squeeze()), torch.mean(loss7.squeeze()))
...
#         prot_rs = prot_embeds[:, -1]
#         prot_rs = F.relu(prot_rs).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    for d in embed_dims:
        ...
        for m in margins:
            ...
            out_file = f'../results/EmEL_dir_2/'+args.data+'_{'+str(embedding_size)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.pkl'
