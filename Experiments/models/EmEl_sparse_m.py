...
class ELModel(nn.Module):
    ...
    def __init__(self, nb_classes, nb_relations, embedding_size, batch_size, margin, reg_norm, width):
        ...
        self.rel_embeddings = nn.Embedding( nb_relations, (embedding_size + 1)*width + 1)
        ...
        rel_sample_weights = np.random.uniform(low=-1, high=1, size=(nb_relations, (embedding_size + 1)*width))
    ...
    def nf3_loss(self, input):
        ...
        c = self.cls_embeddings(c)
        d = self.cls_embeddings(d)
        r_cons = self.rel_const(rel)
        rel = self.rel_embeddings(rel)
        
        r = rel[:, 0:-1]
        r = r.view(-1, embedding_size + 1, self.width)
        epsilon = torch.max(torch.tensor([0.0]).cuda(), rel[:, -1])

        x1 = c[:, 0:-1]
        x2 = d[:, 0:-1]
        x3 = self.transform_vector(c, r, r_cons)
        
        rc = torch.max(torch.tensor([0.0]).cuda(), x3[:, -1])
        rd = torch.max(torch.tensor([0.0]).cuda(), d[:, -1])
        
        euc = torch.norm(x3[:, 0:-1] - x2, dim=1)
    ...
    def nf4_loss(self, input):
        ...
        rc = torch.max(torch.tensor([0.0]).cuda(), x3[:, -1]).view(-1, 1)
        rd = torch.max(torch.tensor([0.0]).cuda(), d[:, -1]).view(-1, 1)
        sr = rc + rd

        dst = torch.norm(x3[:, 0:-1] - x2, dim=1).view(-1, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--width", type=int, action = 'store', help="epochs", default=3)
...
#     set_seed(args)
    ...
    for d in embed_dims:
        ...
        for m in margins:
            ...
            out_file = f'../results/EmEL_dir_2/'+args.data+'_{'+str(embedding_size)+'}_{'+str(args.width)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.pkl'
