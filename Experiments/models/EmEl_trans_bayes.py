...
#         dst = F.relu(torch.max(euc + rc - rd - epsilon, torch.tensor([0.0]).cuda()) + epsilon - self.margin).view(-1, 1)
    ...
    def nf3_loss(self, input):
        ...
        rel = self.rel_embeddings(rel)
        r = rel[:, 0:-1]
        sigma = torch.max(torch.tensor([0.0001]).cuda(), rel[:, -1])
        x1 = c[:, 0:-1]
        x2 = d[:, 0:-1]
        ...
        euc = torch.norm(x3 - x2, dim=1)
        dst = F.relu(((euc + rc - rd)**2)/(2*(sigma**2)) + torch.log(sigma) - self.margin).view(-1, 1)
    ...
    def nf4_loss(self, input):
        ...
        dst_loss = F.relu(((dst - sr)**2)/(2*(sigma**2)) + torch.log(sigma) - self.margin)
    ...
    def nf3_neg_loss(self, input):
        ...
        rel = self.rel_embeddings(rel)
        r = rel[:, 0:-1]
        sigma = torch.max(torch.tensor([0.0]).cuda(), rel[:, -1])
        ...
        euc = torch.norm(x3 - x2, dim=1)
        dst = (-(((euc - rc - rd)**2)/(2*(sigma**2)) + torch.log(sigma)) + self.margin).view(-1, 1)
...
#     set_seed(args)
    ...
    for d in embed_dims:
        ...
        for m in margins:
            ...
            out_file = f'../results/EmEL_dir_4/'+args.data+'_{'+str(embedding_size)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.pkl'
            loss_history_file= f'../results/EmEL_dir_4/'+args.data+'_lossHis_{'+str(embedding_size)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.csv'
