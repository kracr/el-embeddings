...
class ELModel(nn.Module):
    ...
    def __init__(self, nb_classes, nb_relations, embedding_size, batch_size, margin, reg_norm, width):
        ...
        gather_indices = [list(range((r - (width-1)//2), (r + (width-1)//2 + 1))) for r in range(embedding_size+1)]
        sparse_indices = [[],[],[]]
        for b in range(batch_size):
            for row in range(len(gather_indices)):
                for col in gather_indices[row]:
                    sparse_indices[0].append(b)
                    sparse_indices[1].append(row)
                    sparse_indices[2].append(col%(embedding_size+1))
        
        self.sparse_indices = torch.tensor(sparse_indices)
#         self.gather_indices = torch.tensor(gather_indices, dtype=torch.int)
        
        self.inf = 5.0 # For top radius
        self.cls_embeddings = nn.Embedding( nb_classes, embedding_size + 1)
        self.rel_embeddings = nn.Embedding( nb_relations, (embedding_size + 1)*width)
        self.rel_const = nn.Embedding(nb_relations, embedding_size + 1)
        ...
        rel_weights = np.random.uniform(low=-1, high=1, size=(nb_relations, (embedding_size + 1)*width))
        rel_weights = rel_weights / np.linalg.norm(
            rel_weights, axis=1).reshape(-1, 1)
        const_emb_wts = np.random.uniform(low=-1, high=1, size=(nb_relations, embedding_size))
        const_radius_wts = np.random.uniform(low=0, high=1, size=(nb_relations,))
        rel_constants = np.column_stack((const_emb_wts,const_radius_wts)) 
        rel_constants = rel_constants / np.linalg.norm(
            rel_constants, axis=1).reshape(-1, 1)
        ...
        self.rel_const.weight.data.copy_(torch.from_numpy(rel_constants))

    def transform_vector(self, v, matrix_params, bias_params):
        # v: vector of dim d+1 to be transformed
        # matrix_params: (b * d+1 * w) tensor
        d = self.embedding_size
        w = self.width
        v = torch.unsqueeze(v, 2)
        matrix_params = matrix_params.view(-1)
        
        sparse_rel_matrix = torch.sparse_coo_tensor(indices = self.sparse_indices, values = matrix_params, size=(self.batch_size, d+1,d+1)).to_dense()
        
        v = torch.matmul(sparse_rel_matrix, v).squeeze()

        # torch.sum(kernel_w_patches * matrix_params, dim=1) acts as identity when
        # matrix_params is 'identity_tensor' as defined above
        return v + bias_params
...
#         r = self.rel_embeddings(r)
        rad_c = torch.max(torch.tensor([0.0]).cuda(), c[:, -1])
        rad_d = torch.max(torch.tensor([0.0]).cuda(), d[:, -1])
        
        x1 = c[:, 0:-1]
        x2 = d[:, 0:-1]
#         x3 = x1 + r
        
        euc = torch.norm(x1 - x2, dim=1)
        dst = F.relu(euc + rad_c - rad_d - self.margin).view(-1, 1)
    ...
    def nf3_loss(self, input):
        ...
        c = self.cls_embeddings(c)
        d = self.cls_embeddings(d)
        r_cons = self.rel_const(r)
        r = self.rel_embeddings(r)
        rm = r.view(-1, embedding_size + 1, self.width)
        
        c = self.transform_vector(c, rm, r_cons)
        d = self.transform_vector(d, rm, r_cons)
#         r = (r1, r2)
#         cr1 = e = dr2
#         cr* = d
    ...
    def nf4_loss(self, input):
        ...
        rad_c = torch.max(torch.tensor([0.0]).cuda(), c[:, -1]).view(-1, 1)
        rad_d = torch.max(torch.tensor([0.0]).cuda(), d[:, -1]).view(-1, 1)
        sr = rad_c + rad_d

        # c - r should intersect with d
#         x3 = x1 - r
        
        dst = torch.norm(x1 - x2, dim=1).view(-1, 1)
    ...
    def nf3_neg_loss(self, input):
        ...
        r_cons = self.rel_const(r)
        r = self.rel_embeddings(r)
        #transformation of centres and radius in relation space
        rm = r.view(-1, embedding_size + 1, self.width)
...
def build_model(train_data,classes,relations,valid_data, margin, embedding_size, batch_size, reg_norm, learning_rate, epochs, out_file, dataset, width):
    ...
    model = ELModel(nb_classes, nb_relations, embedding_size, batch_size, margin, reg_norm, width).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--width", type=int, action = 'store', help="epochs", default=5)
...
#     set_seed(args)
    ...
    for d in embed_dims:
        ...
        for m in margins:
            ...
            out_file = f'../results/EmEL_dir_3/'+args.data+'_{'+str(embedding_size)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.pkl'
            loss_history_file= f'../results/EmEL_dir_2/'+args.data+'_lossHis_{'+str(embedding_size)+'}_{'+str(margin)+'}_{'+str(epochs)+'}.csv'
            build_model(train_data_model,classes,relations,valid_data_model, margin, embedding_size, batch_size, reg_norm, learning_rate, epochs, out_file, args.data, args.width)
