...
class ELModel(nn.Module):
    ...
    def __init__(self, nb_classes, nb_relations, embedding_size, batch_size, margin, reg_norm):
        ...
        self.rel_embeddings = nn.Embedding( nb_relations, (embedding_size + 1)**2)
        ...
        rel_weights = np.random.uniform(low=-1, high=1, size=(nb_relations, (embedding_size + 1)**2))
...
#         x3 = x1 + r
    ...
    def nf3_loss(self, input):
        ...
        c = torch.matmul(rm, torch.unsqueeze(c, 2)).squeeze()
        d = torch.matmul(rm, torch.unsqueeze(d, 2)).squeeze()
        c += r_cons
        d += r_cons
